...
class Generator(torch.nn.Module):
    ...
    def forward(self, x, spk_embedding=None):
        if self.use_vae:
            b, c, t = x.shape
            spk_embedding = torch.unsqueeze(spk_embedding, -1)
            spk_embedding = spk_embedding.repeat(1, 1, t)
            x = torch.cat((x, spk_embedding), dim=1)
    ...
    def inference(self, x):
        ...
        if self.use_vae:
            b, c, t = x.shape
            spk_embedding = torch.zeros((b, 48), device=x.device)
        return self.forward(x, spk_embedding)
